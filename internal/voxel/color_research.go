...
// Σ w_i p_i = (target_R, target_G, target_B) and Σ w_i = 1, via
// solving the normal equations of the constrained least-squares
// problem. Weights may be negative (input outside convex hull).
func simplexBarycentric(pal [][3]uint8, tR, tG, tB float64, w []float64) {
	K := len(pal)
	// We want to minimize |A w - b|² subject to 1^T w = 1,
	// where A is 3×K palette matrix (rows R,G,B) and b is target.
	//
	// Lagrangian: w = (A^T A)^{-1} A^T b + λ (A^T A)^{-1} 1
	// with λ chosen so that 1^T w = 1.
	//
	// For K=4 in 3D this is the exact barycentric (since 4 unknowns,
	// 3 equations + sum=1 constraint = 4 equations, system invertible
	// if palette spans 3D). For K=2 it's a least-squares projection
	// onto the line; etc.
	//
	// Implementation: form the (K+1)×(K+1) augmented system or just
	// reduce. Simpler approach for any K:
	//   1. Compute unconstrained x = A^T A x = A^T b (K-dim).
	//   2. Add Lagrange constraint via x' = x - (((1^T x) - 1) / (1^T (A^T A)^{-1} 1)) · ((A^T A)^{-1} 1).
	// But this requires an inverse. For our small K=4 we can just
	// solve the (K+1)×(K+1) system directly.
	//
	// Here we use a pragmatic approach: solve the (3+1)=4-row system
	// when K=4 (exact). For other K, we reformulate to use only
	// K-1 free variables (since w_K = 1 - Σ_{i<K} w_i).
	if K == 4 {
		// Exact 4×4 solve: rows = [R, G, B, sum=1].
		var M [4][4]float64
		var rhs [4]float64
		for j := 0; j < 4; j++ {
		...
		rhs[2] = tB
		rhs[3] = 1
		// Gaussian elimination with partial pivoting on a 4×4.
		solve4x4(M, rhs, w)
		return
	}
	// General K: substitute w_{K-1} = 1 - sum(w_{0..K-2}) and solve
	// 3 equations in K-1 unknowns.
	last := K - 1
	...
	// Min-norm w via normal equations: (A A^T) μ = b, w = A^T μ.
	// A is 3×(K-1) when columns are A[j]. Wait we built it row-major.
	// Let me redo: A^T is (K-1)×3. We want least-squares: minimize
	// |A^T w' - b|² where w' is K-1 dim and A^T is matrix with rows
	// A[j].T. Actually simpler: stack A[j] as rows of a (K-1)×3
	// matrix M_rows. We want M_rows^T · w' = b, i.e. M_rows.T · w' = b
	// where w' is a column vector. The normal equations are
	// (M_rows · M_rows^T) · w' = M_rows · b.
	M := make([][]float64, last)
	for i := 0; i < last; i++ {
	...
	// Solve AAt · w' = rhs (Gaussian elimination, no pivoting since
	// AAt is symmetric positive-definite when palette colors are
...
// clipAndRenormalize sets negative weights to 0 and renormalizes
// the remaining positive weights to sum to 1. Single pass — for
// palettes where one round isn't enough we accept a small drift.
func clipAndRenormalize(w []float64) {
	...
	if sum <= 0 {
		// Degenerate — pick the closest by giving first weight 1.
		w[0] = 1
		for i := 1; i < len(w); i++ {
			w[i] = 0
		}
