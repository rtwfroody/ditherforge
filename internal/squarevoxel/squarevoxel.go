...
// PreparedData holds the geometry results from Prepare, reusable across
// multiple Render calls with different color options.
type PreparedData struct {
	...
	CellSize      float32
	LayerH        float32
	DecimModel    *loader.LoadedModel // decimated model for clipping
}
...
func Prepare(ctx context.Context, model *loader.LoadedModel, cfg Config, cached *CacheData) (*PreparedData, *CacheData, error) {
	...
	// Decimate the input mesh. All color info has been extracted into
	// the voxel grid, so we can simplify purely for geometry.
	decimModel := model
	if !cfg.NoSimplify {
	...
	return prepared, newCacheData, nil
}
...
func Render(ctx context.Context, prepared *PreparedData, model *loader.LoadedModel, pcfg voxel.PaletteConfig, cfg Config, ditherMode string) (*loader.LoadedModel, []int32, [][3]uint8, error) {
	...
	// Resolve palette and assign / dither.
	...
	return outModel, shellAssignments, pal, nil
}
...
func Remesh(ctx context.Context, model *loader.LoadedModel, pcfg voxel.PaletteConfig, cfg Config, ditherMode string, cached *CacheData) (*loader.LoadedModel, []int32, [][3]uint8, *CacheData, error) {
	...
	return outModel, assignments, pal, newCache, nil
}
