          ...
          <div class="space-y-4">
            <div class="space-y-2">
              <div class="flex items-center gap-1.5">
                ...
                <HelpTip>
                  "Riemersma" walks cells along a locally-coherent tour through the surface and diffuses each cell's error into a sliding window of recent cells — preserves chroma without scanline directionality. "dizzy-corrected" is a randomized error-diffusion (Liam Appelbe's blue-noise dizzy, iterated three times with drift correction) — produces a blue-noise look with no directional structure on flat areas, at 3× the cost of a single dither pass. "dizzy-2hop" is single-pass dizzy with an expanded 2-hop neighbor stencil so error can still spread when 1-hop neighbors are all already processed (experimental — reduces dizzy's chroma drift without the iteration cost of dizzy-corrected). "dizzy-recover" is single-pass dizzy that, when a cell has no unprocessed neighbors, searches its already-processed neighbors for a palette swap that locally absorbs the residual error (experimental). "Floyd-Steinberg" uses a deterministic scanline order that preserves average chroma exactly, at the cost of visible directional structure on flat areas. "none" disables dithering and snaps each cell to the nearest palette color.
                </HelpTip>
              </div>
